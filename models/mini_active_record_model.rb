...
class Model
  attr_reader :attributes, :old_attributes
    ...
    def initialize(attributes = {})
      #Pusimos sel.class porque se repite el codifo en los des progrmas... initialize
    ...
    def self.where(query, *args)
      MiniActiveRecord::Model.execute("SELECT * FROM #{self.class}s WHERE #{query} = ?", *args).map do |row|
        self.class.new(row)
      end
    end
